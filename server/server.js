...
const startServer = async () => {
  try {
    const server = app.listen(PORT, () => {
      ...
      // Warm scraper cache at startup to speed up first user search.
      const warmStatus = medicineScraperService.startBackgroundScrape({
        reason: 'server_startup'
      });
      console.log(`🧠 Scraper warm-up: ${warmStatus.started ? 'started' : 'already running'}, cached=${warmStatus.cachedCount}`);
    });
