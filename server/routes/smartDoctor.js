...
function getGenAIClient() {
  ...
  // Quickstart style: client reads GEMINI_API_KEY from environment.
...
function classifyUrgencyLevel(message = '') {
  const text = String(message).toLowerCase();

  const emergencyTerms = [
    'chest pain', 'can not breathe', 'cant breathe', 'difficulty breathing',
    'faint', 'unconscious', 'stroke', 'seizure', 'severe bleeding', 'suicidal'
  ];
  const urgentTerms = [
    'high fever', 'worsening', 'severe pain', 'vomiting blood', 'blood in stool',
    'shortness of breath', 'very dizzy', 'persistent vomiting'
  ];

  if (emergencyTerms.some(term => text.includes(term))) return 'high';
  if (urgentTerms.some(term => text.includes(term))) return 'moderate';
  return 'low';
}

function getContextName(contextKey = 'general') {
  const names = {
    cardiology: 'Cardiology & Heart Conditions',
    neurology: 'Neurology & Brain Health',
  ...
  return names[contextKey] || names.general;
}

function buildChatPrompt({ context, history, message, urgency }) {
  const conversation = history
    .slice(-CHAT_HISTORY_TURNS * 2)
    .map((item) => `${item.role === 'assistant' ? 'Doctor' : 'Patient'}: ${item.content}`)
    .join('\n');

  return `You are Dr. Medzy AI, a careful and empathetic medical triage assistant.

Conversation style rules:
- Sound like a calm human clinician in chat, not a report generator.
- Use warm, natural language and acknowledge feelings when appropriate.
- Keep most replies to 4-8 short paragraphs or bullets.
- Ask one focused follow-up question when key details are missing.
- Do not claim a definitive diagnosis.
- If risk is high, prioritize emergency action in the first 1-2 lines.
- Mention medicine safety and consulting a licensed doctor when needed.

Context: ${getContextName(context)}
Urgency signal from triage classifier: ${urgency}
...
Response structure (natural, not rigid headings):
- Brief understanding of the patient concern
- Most likely possibilities (plain language)
- Practical next actions right now
- Red flags and when to seek urgent care
- One short follow-up question

End with one-line safety disclaimer.`;
}

function isGreetingOnly(message = '') {
  const text = String(message).toLowerCase().trim();
  const greetings = ['hi', 'hello', 'hey', 'assalamu alaikum', 'salam', 'good morning', 'good evening'];
...
function getGeminiModelCandidates(model) {
  ...
  const candidates = [preferred, fromEnv, 'gemini-3-flash-preview', 'gemini-2.5-flash', 'gemini-flash-latest'];
...
function getGeminiErrorInfo(error) {
  ...
  if (lower.includes('429') || lower.includes('resource_exhausted') || lower.includes('quota')) {
    return { type: 'quota', message: 'Gemini quota or rate limit exceeded (429).' };
  }
  if (lower.includes('401') || lower.includes('403') || lower.includes('permission') || lower.includes('api key')) {
    return { type: 'auth', message: 'Gemini authentication/permission issue (check API key and project access).' };
  }
  if (lower.includes('404') || lower.includes('not found') || lower.includes('model')) {
    return { type: 'model', message: 'Requested Gemini model is unavailable for this key/project.' };
  }
  if (lower.includes('400') || lower.includes('invalid')) {
    return { type: 'request', message: 'Gemini request was invalid for this model/config.' };
  }
  if (lower.includes('503') || lower.includes('unavailable') || lower.includes('timeout')) {
    return { type: 'transient', message: 'Gemini service is temporarily unavailable.' };
  }

  return { type: 'unknown', message };
}

async function queryGeminiChat(prompt, model = 'gemini-3-flash-preview') {
    ...
    try {
      for (let attempt = 0; attempt <= transientRetries; attempt++) {
        try {
          ...
          if (!text) {
            throw new Error(`Gemini returned an empty response for ${candidateModel}`);
          }

          return text;
        } catch (error) {
          const info = getGeminiErrorInfo(error);
          const canRetry = (info.type === 'quota' || info.type === 'transient') && attempt < transientRetries;
      ...
      lastError = error;
      const info = getGeminiErrorInfo(error);
      const shouldTryNextModel = info.type === 'model' || info.type === 'request';
...
function buildFallbackChatResponse(message, context = 'general') {
  const analysis = analyzeSymptomPatterns(String(message).toLowerCase());

  return `Thanks for sharing that. I understand this can feel stressful.

From what you described, the more likely possibilities are:
${analysis.conditions.map(c => `- ${c.name} (${c.probability}% likelihood)`).join('\n') || '- A non-specific symptom pattern that should be monitored closely'}

What you can do right now:
${analysis.selfCare.map(item => `- ${item}`).join('\n') || '- Rest, hydrate, and track symptom changes every 6-8 hours'}

Please watch for these warning signs:
${analysis.redFlags.map(flag => `- ${flag}`).join('\n') || '- Severe or rapidly worsening symptoms'}

If you are not improving within ${analysis.followUp.timeframe}, book a doctor visit. If severe symptoms appear, seek emergency care.

Quick follow-up: when did this start, and is it getting better, worse, or staying the same?

Disclaimer: This is informational guidance only and not a medical diagnosis.`;
}

function buildFollowUpSuggestions(context = 'general', urgency = 'low') {
  if (urgency === 'high') {
    return [
      'What emergency signs should I watch for in the next 1 hour?',
      'What should I tell ER staff first?',
      'Can I safely take any medicine before reaching a hospital?'
    ];
  }

  const base = {
    respiratory: ['How can I reduce cough at night?', 'Which symptoms suggest pneumonia?', 'When do I need a chest X-ray?'],
    cardiology: ['How to track chest discomfort safely?', 'What BP readings are dangerous?', 'When should I do an ECG?'],
    neurology: ['What headache signs are dangerous?', 'How should I track migraine triggers?', 'When should I see a neurologist?'],
    general: ['Which symptom changes should I monitor today?', 'What OTC options are usually safer?', 'When should I book a doctor visit?']
  };

  return base[context] || base.general;
}

// Minimal Gemini chatbot endpoint (simple, direct, no heuristic fallback)
router.post('/simple-chat', async (req, res) => {
  try {
    const { message, model = process.env.GEMINI_MODEL || 'gemini-3-flash-preview' } = req.body || {};
    ...
    if (!genAI) {
      return res.status(500).json({
        ...
        provider: 'gemini',
        errorType: 'auth',
        message: 'Gemini API key is missing. Set GEMINI_API_KEY in .env and restart server.'
      });
    }
...
router.post('/chat', authenticateToken, async (req, res) => {
  try {
    const { message, context = 'general', model = 'gemini-2.0-flash', reset = false } = req.body || {};

    if (!message || !String(message).trim()) {
      return res.status(400).json({ success: false, message: 'Message is required' });
    }

    const userId = String(req.user.id || req.user.userId || 'unknown-user');
    if (reset) {
      chatSessions.delete(userId);
    }

    const history = (chatSessions.get(userId) || []).slice(-CHAT_HISTORY_TURNS * 2);
    const urgency = classifyUrgencyLevel(message);

    if (isGreetingOnly(message)) {
      return res.json({
        success: true,
        response: `Hi ${req.user?.firstName || ''}! I am here with you. Tell me what you are feeling right now, and I will help step by step.`,
        urgency: 'low',
        provider: 'assistant',
        suggestions: [
          'I have headache and nausea since morning',
          'I have fever and sore throat for 2 days',
          'I feel chest discomfort when walking'
        ],
        timestamp: new Date().toISOString()
      });
    }
    ...
    try {
      const prompt = buildChatPrompt({ context, history, message: String(message).trim(), urgency });
      responseText = await queryGeminiChat(prompt, model);
      provider = 'gemini';
    } catch (aiError) {
      providerError = getGeminiErrorInfo(aiError);
      console.warn('Smart doctor chat using fallback:', providerError.message);
      responseText = buildFallbackChatResponse(message, context);
    }
    ...
    chatSessions.set(userId, updatedHistory);
    ...
    res.json({
      ...
      provider,
      providerError,
      suggestions: buildFollowUpSuggestions(context, urgency),
      timestamp: new Date().toISOString()
    });
  } catch (error) {
    console.error('Smart doctor chat error:', error);
    res.status(500).json({
      success: false,
  ...
  upload.single('prescriptionImage')(req, res, async (err) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error('Failed to process image with OCR: ' + tesseractError.message);
      }

      // Validate extracted text
      if (!text || text.trim().length === 0) {
        throw new Error('No text could be extracted from the prescription image. Please try with a clearer image.');
      }
