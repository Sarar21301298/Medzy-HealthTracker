...
const cartItemSchema = new mongoose.Schema({
  ...
  // For scraped medicines
  scrapedMedicineId: {
    type: String  // MD5 hash of medicine name + source
  },
  scrapedMedicineData: {
    name: String,
    manufacturer: String,
  ...
  // Type indicates which field is used
  type: {
    type: String,
    enum: ['regular', 'scraped'],
    required: true
  },
  quantity: {
    type: Number,
  ...
  // Custom validation to ensure at least one medicine reference exists
  validate: {
    validator: function() {
      return (this.type === 'regular' && this.medicine) || (this.type === 'scraped' && this.scrapedMedicineId);
    },
    message: 'Either medicine or scrapedMedicineId must be provided based on type'
  }
});
...
const userSchema = new mongoose.Schema({
  ...
  role: {
    type: String,
    required: true,
    enum: ['admin', 'customer', 'pharmacy_vendor', 'rider'],
    default: 'customer'
  },
  firstName: {
    type: String,
